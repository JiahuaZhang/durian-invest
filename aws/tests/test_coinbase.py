...
def test_coinbase_feed_live():
    """
    Real test that connects to Coinbase using config,
    subscribes, and logs price updates for 3 seconds.
    """
    asyncio.run(_test_coinbase_feed_live())

async def _test_coinbase_feed_live():
    ...
    # 3. Initialize feed
    feed = CoinbaseFeed(
        product=config.feeds.coinbase_product,
        on_update=on_update
    )
    ...
    logger.info("Listening for Coinbase price updates for 3 seconds...")
    ...
    assert len(updates_received) > 0, "No price updates received from Coinbase within 3 seconds."
    logger.info(f"Test complete. Successfully received {len(updates_received)} updates in 3 seconds.")

# $env:PYTHONPATH="."; uv run pytest tests/test_coinbase.py -s -v --log-cli-level=INFO
